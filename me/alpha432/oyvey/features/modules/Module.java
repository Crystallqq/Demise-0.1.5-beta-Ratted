...
public class Module extends Feature
{
    ...
    public void enable() {
        ...
        if (HUD.getInstance().notifyToggles.getValue()) {
            final ITextComponent text = (ITextComponent)new TextComponentString(OyVey.commandManager.getClientMessage() + " " + ChatFormatting.GREEN + this.getDisplayName() + " toggled on.");
    ...
    public void disable() {
        ...
        if (HUD.getInstance().notifyToggles.getValue()) {
            final ITextComponent text = (ITextComponent)new TextComponentString(OyVey.commandManager.getClientMessage() + " " + ChatFormatting.RED + this.getDisplayName() + " toggled off.");
            Module.mc.ingameGUI.getChatGUI().printChatMessageWithOptionalDeletion(text, 1);
        }
        this.onToggle();
        this.onDisable();
