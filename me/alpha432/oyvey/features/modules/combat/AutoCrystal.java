...
public class AutoCrystal extends Module
{
    ...
    public void onCrystal() {
        ...
        if (this.placeTimer.passedMs(this.placeDelay.getValue().longValue()) && this.place.getValue()) {
            ...
            for (final BlockPos blockPos : this.placePostions(this.placeRange.getValue())) {
                ...
                if (!AutoCrystal.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(blockPos)).isEmpty()) {
                    continue;
                }
                final double targetRange = this.target.getDistance((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ());
                ...
                Label_1212: {
                    if (targetDmg < this.minDamage.getValue()) {
                        ...
                        if (this.facePlaceSword.getValue()) {
                            ...
                            if (AutoCrystal.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword) {
                                continue;
                            }
                            if (!this.armor) {
                                continue;
                ...
                final double selfDmg = this.calculateDamage(blockPos.getX() + 0.5, blockPos.getY() + 1.0, blockPos.getZ() + 0.5, (Entity)AutoCrystal.mc.player);
                if (selfDmg + (this.suicide.getValue() ? 2.0 : 0.5) >= AutoCrystal.mc.player.getHealth() + AutoCrystal.mc.player.getAbsorptionAmount() && selfDmg >= targetDmg && targetDmg < this.target.getHealth() + this.target.getAbsorptionAmount()) {
                    continue;
                }
    ...
    public void onPacketReceive(final PacketEvent.Receive event) {
        if (event.getPacket() instanceof SPacketSpawnObject) {
            ...
            if (packet.getType() == 51 && this.predicts.getValue() && this.preditTimer.passedMs(this.attackFactor.getValue()) && this.predicts.getValue() && this.explode.getValue() && this.packetBreak.getValue() && this.target != null) {
    ...
    private boolean isPredicting(final SPacketSpawnObject packet) {
        ...
        if (selfDmg + (this.suicide.getValue() ? 2.0 : 0.5) < AutoCrystal.mc.player.getHealth() + AutoCrystal.mc.player.getAbsorptionAmount() && targetDmg >= this.target.getAbsorptionAmount() + this.target.getHealth()) {
            return true;
        }
        this.armorTarget = false;
        for (final ItemStack is : this.target.getArmorInventoryList()) {
    ...
    EntityPlayer getTarget() {
        ...
        for (final EntityPlayer entity : AutoCrystal.mc.world.playerEntities) {
            ...
            if (AutoCrystal.mc.player.isDead) {
                continue;
            }
            if (entity.isDead) {
                continue;
