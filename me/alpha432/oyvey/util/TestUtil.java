...
public class TestUtil
{
    ...
    public static boolean isBlockEmpty(final BlockPos pos) {
        try {
            if (TestUtil.emptyBlocks.contains(TestUtil.mc.world.getBlockState(pos).getBlock())) {
                ...
                for (final Entity e : TestUtil.mc.world.loadedEntityList) {
    ...
    public static boolean canPlaceBlock(final BlockPos pos) {
        if (isBlockEmpty(pos)) {
            ...
            final EnumFacing[] facings = values = EnumFacing.values();
            for (final EnumFacing f : values) {
                if (!TestUtil.emptyBlocks.contains(TestUtil.mc.world.getBlockState(pos.offset(f)).getBlock()) && TestUtil.mc.player.getPositionEyes(TestUtil.mc.getRenderPartialTicks()).distanceTo(new Vec3d(pos.getX() + 0.5 + f.getXOffset() * 0.5, pos.getY() + 0.5 + f.getYOffset() * 0.5, pos.getZ() + 0.5 + f.getZOffset() * 0.5)) <= 4.25) {
                    return true;
                }
            }
        }
